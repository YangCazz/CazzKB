...
它不是一个简单的聊天壳，而是一个完整的 RAG 工作台：文档解析、语义分块、混合检索、精排、流式生成和 React 工作台界面都收敛在一个可自部署仓库里。
...
1. 导入 Markdown 笔记、博客文章、技术资料或项目文档。
2. 解析 frontmatter，保护代码块/表格/公式，并切分为语义片段。
3. 使用 Ollama 与 `bge-m3` 构建本地 embedding，再与 BM25 结果融合。
4. 对候选片段进行 Cross-Encoder 精排，并从配置的 LLM provider 流式生成回答。
5. 在聊天工作台中查看回答、上下文和可编辑的对话历史。

## 项目展示点

- 全栈 AI 产品工程：FastAPI 检索后端 + React 19 工作台 UI。
- 超越基础向量检索：BM25、RRF 融合与 Cross-Encoder 精排。
- Local-first 系统设计：适合私有知识工作流和可复现实验。
